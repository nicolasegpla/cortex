"""Minimal TDD coverage for wine producers router wiring."""
...
class TestWineProducersWiring:
    ...
    def test_create_app_includes_wine_producer_routes(self) -> None:
        ...
        assert ("/wine-producers", "POST") in route_map
        assert ("/wine-producers", "GET") in route_map
        assert ("/wine-producers/{producer_id}", "GET") in route_map
        assert ("/wine-producers/{producer_id}", "PUT") in route_map
        assert ("/wine-producers/{producer_id}", "DELETE") in route_map

    def test_delete_wine_producer_for_operativo_returns_403(
        ...
        monkeypatch.setattr(WineProducerService, "delete", mock_delete)
        monkeypatch.setattr(WineProducerService, "list_all", mock_list_all)
