...
The system MUST allow a user to add, update, remove, and test their own provider credentials for OpenAI, Kimi, Minimax, and Anthropic from the frontend workspace.
...
- AND the workspace can later use it for chat without requiring engineering intervention
...
The system MUST let the user validate a provider credential and SHALL present provider-specific ready, invalid, or failed status in a way that guides chat usage.

#### Scenario: Credential validates successfully
- GIVEN a user tests a valid provider credential
- WHEN validation completes
- THEN the workspace shows that the provider is ready for chat
