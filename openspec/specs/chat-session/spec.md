...
(Previously: requirement text referenced a legacy `/chat/stream` SSE rollback path and included a stop-generation scenario for that path. The backend `/chat/stream` route is now deleted — the entire legacy streaming path is gone.)
